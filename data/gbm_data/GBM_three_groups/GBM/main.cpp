...
ifstream net1_in("Astnet.txt");
//Read the edge set of the network2
ifstream net2_in("aNSCnet.txt");
//Output the network graphlet to files
ofstream net1_out("Astmatrix.txt");
ofstream net2_out("aNSCmatrix.txt");*/

//Read the graphlet vector of network 1 and 2
ifstream graphlet1_in("GBM_matrix.txt");
ifstream graphlet2_in("Fetal_matrix.txt");
...
/*
    ...
    for(p=0;p<MAXN;p++){
        for(q=0;q<15;q++){
            graphlet1_in>>graphlet1[p][q];
        }
    }

    for(p=0;p<MAXM;p++){
        for(q=0;q<15;q++){
            graphlet2_in>>graphlet2[p][q];
        }
    }

    //¼ÆËãÁ½¸öÍøÂçÍ¼ÔªµÄ²îÏòÁ¿
