...
import src.main.gbm.minus_kds_gbm as kds
...
if __name__ == '__main__':
    ...
    process_logger.info('[[[=================| %d |================]]]', 1)

    # 拼接文件存储路径的前缀
    txt_path = constants.base_path
    process_logger.info('[Read File Base Path]->%s', txt_path)
    # 需要读取的文件
    network1_path = os.path.join(txt_path, 'newnetNPC.txt')
    network2_path = os.path.join(txt_path, 'newnetaNSC.txt')
    ...
    # 读取重要的点
    common_top_nodes = util.prepare_top_nodes(dvalue_path, 0.4, 48)

    # 开始计算1
    start_calculate(network1_path, dvalue_path, subgraph_network_path, subgraph_node_path, common_top_nodes,
                    dvalue_weight,
                    edge_weight)


    # 开始计算 network2
    start_calculate(network2_path, dvalue_path, subgraph2_network_path, subgraph2_node_path, common_top_nodes,
                    dvalue_weight,
                    edge_weight)
